...
class Goal(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), default=GoalStatus.active.value, index=True)
...
class Book(Base):
    ...
    status: Mapped[str] = mapped_column(String(20), default=BookStatus.wishlist.value, index=True)
...
class Task(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    project_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("projects.id"), index=True)
    title: Mapped[str] = mapped_column(String(300), nullable=False)
    description: Mapped[str | None] = mapped_column(Text)
...
class Note(Base):
    ...
    parent_id: Mapped[int | None] = mapped_column(Integer, ForeignKey("notes.id"), nullable=True, index=True)
    tags: Mapped[list[str] | None] = mapped_column(JSON)
...
class Review(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True)
    period_type: Mapped[str | None] = mapped_column(String(10), index=True)
    period_label: Mapped[str | None] = mapped_column(String(30), index=True)
